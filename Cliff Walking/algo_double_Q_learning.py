...
class Double_QLearning:
    ...
    def choose_action(self, state, epsilon):
        ...
        if np.random.uniform(0, 1) < epsilon:
            ...
            state_action = random.shuffle(list(map(add, state_action1, state_action2)))
    ...
    def learning(self, state, action, reward, next_state):
        ...
        if np.random.random() < self.probability:
            if next_state != 'Goal' or next_state != 'Obstacle':
                q_target1 = reward + self.gamma * self.q_table2.loc[next_state, arg1]
            else:
                q_target1 = reward
            self.q_table1.loc[state, action] += self.alpha * (q_target1 - q_current1)
        else:
            if next_state != 'Goal' or next_state != 'Obstacle':
                q_target2 = reward + self.gamma * self.q_table1.loc[next_state, arg2]
            else:
                q_target2 = reward
            self.q_table2.loc[state, action] += self.alpha * (q_target2 - q_current2)

        return self.q_table1.loc[state, action], self.q_table2.loc[state, action]
    ...
    def check_state_exist1(self, state):
        if state not in self.q_table1.index:
            self.q_table1 = self.q_table1.append(pd.Series([0]*len(self.actions), index=self.q_table1.columns, name=state))

    def check_state_exist2(self, state):
        if state not in self.q_table2.index:
            self.q_table2 = self.q_table2.append(pd.Series([0]*len(self.actions), index=self.q_table2.columns, name=state))